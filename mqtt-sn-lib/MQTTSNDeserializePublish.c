...
int MQTTSNDeserialize_publish(unsigned char* dup, int* qos, unsigned char* retained, unsigned short* packetid, MQTTSN_topicid* topic,
		unsigned char** payload, int* payloadlen, unsigned char* buf, size_t buflen)
{
	MQTTSNFlags flags;
	...
	enddata = buf + mylen;
	if (enddata - curdata > (int) buflen)
		goto exit;

	if (readChar(&curdata) != MQTTSN_PUBLISH)
		goto exit;

	flags.all = (unsigned char) readChar(&curdata);
	*dup = flags.bits.dup;
	*qos = flags.bits.QoS;
		...
		unsigned char* returncode, unsigned char* buf, size_t buflen)
{
	...
	*topicid = (short unsigned int) readInt(&curdata);
	*packetid = (short unsigned int) readInt(&curdata);
	*returncode = (unsigned char) readChar(&curdata);
...
/**
  ...
  */
int MQTTSNDeserialize_ack(unsigned char* type, unsigned short* packetid, unsigned char* buf, size_t buflen)
{
	unsigned char* curdata = buf;
	unsigned char* enddata = NULL;
	int rc = 0;
	...
	*type = (unsigned char) readChar(&curdata);
		...
		unsigned char* buf, size_t buflen)
{
	...
	*topicid = (unsigned short)readInt(&curdata);
	*packetid = (unsigned short)readInt(&curdata);

	topicname->lenstring.data = (char*)curdata;
	// ! fix this
	topicname->lenstring.len = (size_t) (enddata - curdata);
		...
		unsigned char* buf, size_t buflen)
{
	...
	*topicid = (unsigned short) readInt(&curdata);
	*packetid = (unsigned short) readInt(&curdata);
	*return_code = (unsigned char) readChar(&curdata);
