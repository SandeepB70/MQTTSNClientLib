...
ssize_t transport_sendPacketBuffer(char* host, int port, unsigned char* buf, size_t buflen)
{
...
int transport_getdata(unsigned char* buf, size_t count)
{
	// ! fix this later
	int rc = (int) recvfrom(mysock, buf, count, 0, NULL, NULL);
